...
prediction_years_gdp = 1970:2050;
predicted_per_capita_gdp = a_gdp(1) + a_gdp(2) .* prediction_years_gdp + a_gdp(3) .* prediction_years_gdp.^2 + a_gdp(4) .* prediction_years_gdp.^3;
...
predicted_power_usage = a_power(1) + a_power(2) .* prediction_years_gdp + a_power(3) .* prediction_years_gdp.^2 + a_power(4) .* prediction_years_gdp.^3;
...
figure;
plot(p_gdp, q_gdp(p_gdp), 'b', x_gdp, y_gdp, '*', 'MarkerSize', 2); % GDP per capita plot
hold on;
plot(p_power, q_power(p_power), 'r', x_power, y_power, '*', 'MarkerSize', 2); % Power usage plot
